...
from .naming_preset import SIMPLE_EXPORT_preset
from .. import __package__ as base_package
from .naming_preset import get_preset_folder_path
...
def get_default_preferences(context):
    ...
    attributes = [attr.split('.')[-1] for attr in SIMPLE_EXPORT_preset.preset_values]

    default_prefs = {f"prefs.{attr}": getattr(prefs, attr) for attr in attributes}
    return default_prefs


class UpgradeSimpleExportPresetsOperator(Operator):
    """
    Operator to upgrade old collider tools preset files to the new format.
    """
    bl_idname = "preset.upgrade_simple_export_presets"
    ...
    def execute(self, context):
        ...
        """
        preset_folder = get_preset_folder_path()
    ...
    def upgrade_preset(self, file_path, default_properties):
        ...
        for key, default_value in default_properties.items():
            value = existing_properties.get(key,
                                            f"'{default_value}'" if isinstance(default_value, str) else default_value)
