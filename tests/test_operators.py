...
class TestRemoveExporters(unittest.TestCase):
    ...
    def _make_self(self, collection_name="Assets", outliner=False):
        mock_self = MagicMock()
        mock_self.collection_name = collection_name
        mock_self.outliner = outliner
        return mock_self
    ...
    def test_outliner_removes_from_every_selected_collection(self):
        """outliner=True must remove exporters from every collection returned by
        get_outliner_collections, not just a single collection_name."""
        cols = [_make_collection("A"), _make_collection("B"), _make_collection("C")]
        mock_self = self._make_self(outliner=True)
        with patch.object(_ef, "remove_all_collection_exporters") as mock_remove, \
             patch.object(_remove_mod, "get_outliner_collections", return_value=cols) as mock_get:
            result = SIMPLEEXPORT_OT_remove_exporters.execute(mock_self, MagicMock())

        self.assertEqual(result, {"FINISHED"})
        mock_get.assert_called_once()
        self.assertEqual(mock_remove.call_count, 3)
        mock_remove.assert_any_call(cols[0])
        mock_remove.assert_any_call(cols[1])
        mock_remove.assert_any_call(cols[2])

    def test_outliner_with_no_selection_returns_cancelled(self):
        mock_self = self._make_self(outliner=True)
        with patch.object(_remove_mod, "get_outliner_collections", return_value=[]):
            result = SIMPLEEXPORT_OT_remove_exporters.execute(mock_self, MagicMock())
        self.assertEqual(result, {"CANCELLED"})
